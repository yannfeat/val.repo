...
#' @param final_time The time of the final analysis.
#' @param input_check Indicate whether or not the arguments input by user contain invalid values.
#' @return It returns the expected sample size when 'effect size' is the truth.
#' @seealso
#' \code{\link{adaptive_analysis_norm_global}} for example of this function.
#' @export
exp_sample_size_norm_global <- function(
  initial_test = 0,
  effect_size = 0,
  time = 0,
  final_time = 0,
  ...
  if ( input_check ) {
    if ( time < 0 ) {
      stop("'time' should be non-negative.");
    }
    if ( time < 0 ) {
      stop("'time' should be non-negative.");
    }
    if ( final_time < 0 ) {
      stop("'final_time' should be non-negative.");
    }
    if ( time >= final_time ) {
      stop("'final_time', the time of final analysis, should be larger than 'time', the time of final interim analysis.");
    }
  }

  return( exp_sample_size_norm_c(
    initial_test,
    effect_size,
    time,
    final_time,
