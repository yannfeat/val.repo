...
bar_graph=function(model,
  ...
  if(horiz==TRUE){
    graph=ggplot(data,aes(y=trats,
      ...
      model$plot$theme+
      geom_col(fill=fill,
               color="black",width = model$width.column)+
      geom_errorbar(aes(xmin=media-desvio,
                        xmax=media+desvio),
                    width=model$width.bar)+
      geom_text(aes(x=media+desvio+sup,
                     y=trats,
                     label = letra),hjust=0)+
      labs(y=model$plot$labels$x,
           x=model$plot$labels$y)+
      model$plot$theme+
  ...
  if(horiz==FALSE){
    graph=ggplot(data,aes(x=trats,
                          y=media))+
      geom_col(fill=fill,color="black",width = model$width.column)+
      geom_errorbar(aes(ymin=media-desvio,
                        ymax=media+desvio),width=model$width.bar)+
