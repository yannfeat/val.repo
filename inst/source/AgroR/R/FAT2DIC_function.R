...
#' @param names.fat Name of factors (this argument uses the \emph{parse} function)
...
#' @param lab.factor Provide a vector with two observations referring to the x-axis name of factors 1 and 2, respectively, when there is an isolated effect of the factors. This argument uses `parse`.
...
FAT2DIC=function(f1,
  ...
  lab.factor=c(gsub(" ","~",names.fat[1]),gsub(" ","~",names.fat[2]))
  ...
  { if(print.on==TRUE){
      ...
      if(quali[i]==TRUE){
        ...
        if(fill[1]=="trat"){grafico=grafico+
          ...
          geom_col(aes(fill=trats),
                   fill=fill[1],color=1,width = width.column)}
        grafico=grafico+theme+ylab(parse(text=ylab))+xlab(parse(text = lab.factor[i]))+ylim(ylim)
        ...
        if(length(fill)>=2){grafico=grafico+scale_fill_manual(values=fill)}
        }
    ...
    if(a$`Pr(>F)`[1]>=alpha.f && a$`Pr(>F)`[2] >=alpha.f){
      ...
      if(quali[1]==FALSE && color=="gray"| quali[2]==FALSE && color=="gray"){
        ...
        if(quali[2]==FALSE){
          ...
          grafico=polynomial2(Fator2a,
                              ...
                              SSq=ab$`Sum Sq`[4],
                              DFres = ab$Df[4],
                              legend.title=parse(text = lab.factor[1]),
                              print.on = print.on,
                              plot.on = FALSE)
          if(plot.on==TRUE){print(grafico)}
          if(quali[1]==FALSE & quali[2]==FALSE){
            graf=list(grafico$plot,NA)}
          }
        if(quali[1]==FALSE){
          if (mcomp == "tukey"){
            for (i in 1:nv1) {
        ...
        if(quali[1]==FALSE){
          ...
          grafico=polynomial2(Fator1a,
                              ...
                              SSq=ab$`Sum Sq`[4],
                              DFres = ab$Df[4],
                              legend.title=parse(text = lab.factor[2]),
                              print.on = print.on,
                              plot.on = FALSE)
          if(plot.on==TRUE){print(grafico$plot)}
          if(quali[1]==FALSE & quali[2]==FALSE){
            graf[[2]]=grafico$plot
            grafico=graf}
                    }
      ...
      numero=graph$numero
        ...
        ylab(parse(text=ylab))+xlab(parse(text=lab.factor[1]))+ylim(ylim)+
        theme
      ...
      colint=colint+theme(text=element_text(size=textsize,family = family),
              ...
              legend.position = posi)+labs(fill=parse(text=lab.factor[2]))
  ...
  if(a$`Pr(>F)`[3]>alpha.f){
    ...
    colints=graficos}else{
    colints=list(residplot=residplot,interaction=grafico)}
  output=list(plot=if(any(a$`Pr(>F)`[1:3] < alpha.f)){colints}else{NA},
              dec=dec,
              family=family,
              ylab=ylab,
              textsize=textsize,
