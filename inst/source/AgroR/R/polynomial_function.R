...
#' @param ylab Dependent variable name (this argument uses the \emph{parse} function)
#' @param xlab Independent variable name (this argument uses the \emph{parse} function)
...
polynomial=function(trat,
  ...
  erro=tapply(resp, trat, sd, na.rm=TRUE)/sqrt(tapply(resp, trat, function(x) sum(!is.na(x))))
  dose=tapply(trat, trat, mean, na.rm=TRUE)
  moda=lm(resp~trat)
  ...
  if(point=="mean_sd"){grafico=grafico+
    geom_errorbar(aes(ymin=resp-desvio,ymax=resp+desvio),width=width.bar,linewidth=linesize)}
  if(point=="mean_se"){grafico=grafico+
    geom_errorbar(aes(ymin=resp-erro,ymax=resp+erro),width=width.bar,linewidth=linesize)}
  ...
  grafico=grafico+geom_point(aes(fill=as.factor(rep(1,length(resp)))),na.rm=TRUE,
               ...
               color="black")+
    theme+ylab(parse(text=ylab))+xlab(parse(text=xlab))
  if(is.na(ylim[1])==TRUE){grafico=grafico}else{grafico=grafico+ylim(ylim)}

  if(grau=="0"){grafico=grafico+geom_line(y=mean(resp),linewidth=linesize,lty=2)}
  if(grau=="1"){grafico=grafico+geom_smooth(method = "lm",se=se, na.rm=TRUE, formula = y~x,linewidth=linesize,color="black")}
  if(grau=="2"){grafico=grafico+geom_smooth(method = "lm",se=se, na.rm=TRUE, formula = y~x+I(x^2),linewidth=linesize,color="black")}
  if(grau=="3"){grafico=grafico+geom_smooth(method = "lm",se=se, na.rm=TRUE, formula = y~x+I(x^2)+I(x^3),linewidth=linesize,color="black")}
  ...
  grafico=grafico+
    theme(text = element_text(size=textsize,color="black",family=family),
          ...
          legend.text=element_text(size=textsize,hjust=0),
