...
PSUBDIC=function(f1,
  ...
  ylab=parse(text=gsub(" ","~",ylab))
        ...
        {if(print.on==TRUE){
          cat(green(bold("\n-----------------------------------------------------------------\n")))
            ...
            if(quali[i]==FALSE){
                ...
                # dose=as.numeric(as.vector(unlist(fata[i])))
            ...
            grafico=polynomial(dose,
                               ...
                               family=family,
                               DFres = num(tab[2*i,1]),
                               SSq=num(tab[2*i,2]),print.on = print.on,
                               plot.on = FALSE)
      ...
      if(print.on==TRUE){
        ...
        if(quali[1]==TRUE & quali[2]==TRUE){
            ...
            colint=ggplot(graph, aes(x=f1,
                ...
                ylab(parse(text=ylab))+xlab(parse(text = lab.factor[2]))+theme+ylim(ylim)
        ...
        if(quali[1]==FALSE  && color=="gray"| quali[2]==FALSE && color=="gray"){
            ...
            if(quali[2]==FALSE){
                fator2a=as.numeric(as.character(fator2))
            ...
            if(quali[1]==FALSE){
                fator1a=as.numeric(as.character(fator1))
        ...
        if(quali[1]==FALSE && color=="rainbow"| quali[2]==FALSE && color=="rainbow"){
            ...
            if(quali[1]==FALSE){
                ...
                grafico=polynomial2_color(fator1a,
                                          ...
                                          grau = grau12,
                                          ylab=ylab,
                                          xlab=parse(text=lab.factor[1]),legend.title=parse(text=lab.factor[2]),                                          theme=theme,
                                          point=point,
                                          posi=posi,
                                          ylim=ylim,
                                          textsize=textsize,
    ...
    if(as.numeric(tab[4, 5])>alpha.f){
        ...
        colints=graficos}else{colints=list(residplot=residplot,interaction=colints)}
    output=list(plot=if(any(tab$`Pr(>F)`[c(1,3,4)] < alpha.f)){colints}else{NA},
                dec=dec,
                family=family,
                ylab=ylab,
                textsize=textsize,
