...
bargraph_twofactor=function(analysis,
  ...
  for(i in 1:length(analysis)){
    if(is.null(labels)==TRUE){analysis[[i]][[2]]$plot$graph$facet=rep(i,
                                                                      e=nrow(analysis[[i]]$plot[[2]]$data))}else{
                                                                        analysis[[i]]$plot[[2]]$data$facet=rep(labels[i],
                                                                                                               e=nrow(analysis[[i]]$plot[[2]]$data))}
    results[[i]]=analysis[[i]]$plot[[2]]$data}
  ...
  graph=ggplot(tabela,aes(y=media,x=f1,fill=f2))+
    ...
    analysis$plot[[1]]$theme+theme(strip.text = element_text(size=facet.size))
  if(is.null(ylab)==TRUE){graph=graph+ylab(analysis[[1]]$ylab)}else{graph=graph+ylab(ylab)}
