...
summarise_dunnett=function(variable, colnames=NA, info="sig"){
  ...
  nomes=rownames(variable[[1]]$plot$data)
  ...
  for(i in 1:variaveis){
    if(info=="sig"){datas[,i]=variable[[i]]$plot$data$sig}
    if(info=="t"){datas[,i]=variable[[i]]$plot$data$t.value}
    if(info=="p"){datas[,i]=variable[[i]]$plot$data$p.value}
    if(info=="estimate"){datas[,i]=variable[[i]]$plot$data$Estimate}
    if(info=="IC.lower"){datas[,i]=variable[[i]]$plot$data$IC.lwr}
    if(info=="IC.upper"){datas[,i]=variable[[i]]$plot$data$IC.upr}
  }
