...
FAT2DBC=function(f1,
  ...
  if (a$`Pr(>F)`[4] > alpha.f){
    ...
    for (i in 1:2) {if (a$`Pr(>F)`[i] <= alpha.f){
      ...
      if(geom=="bar"){grafico=ggplot(dadosm,aes(x=trats,
                                                y=media))
      if(fill[1]=="trat"){grafico=grafico+geom_col(aes(fill=trats),
                                                color=1,width = width.column)}
      else{grafico=grafico+geom_col(aes(fill=trats),fill=fill[1],color=1,width = width.column)}
  ...
  output=list(plot=if(any(a$`Pr(>F)`[c(1,2,4)] < alpha.f)){colints}else{NA},
              dec=dec,
              family=family,
              ylab=ylab,
              textsize=textsize,
