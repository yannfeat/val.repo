...
PSUBDBC=function(f1,
    ...
    {if(print.on==TRUE){
      ...
      if(quali[i]==TRUE){
        ...
        if(fill[1]=="trat"){grafico=grafico+
          ...
          geom_point(aes(color=trats),color=fill[1],size=4)}
        grafico=grafico+
          theme+
          ylab(parse(text=ylab))+
          xlab(parse(text = lab.factor[i]))
      ...
      if(quali[i]==FALSE){
        ...
        grafico=polynomial(dose,
                           ...
                           SSq=num(tab[3*i,2]),print.on = print.on,
                           plot.on = FALSE)
  ...
  if (as.numeric(tab[5, 5]) <= alpha.f) {
    ...
    if(quali[1]==TRUE & quali[2]==TRUE){
        ...
        colint=ggplot(graph, aes(x=f1,
          ...
          ylab(parse(text=ylab))+xlab(parse(text = lab.factor[2]))+ylim(ylim)+
          theme
  ...
  output=list(plot=if(any(tab$`Pr(>F)`[c(1,4,5)] < alpha.f)){colints}else{NA},
              dec=dec,
              family=family,
              ylab=ylab,
              textsize=textsize,
