...
#'
#' @param output Output from a FAT2DIC.ad or FAT2DBC.ad function (Use quantity factor as f2).
...
desd_fat2_quant_ad=function(output,
                            ...
                            design="FAT2DIC.ad",
                            grau=1){
  alpha.f=output$alpha.f
  alpha.t=output$alpha.t
  grau21=output$grau21
  ylab=output$ylab
  xlab=parse(text=output$xlab.factor[2])
  posi=output$posi
  theme=output$plot$theme
  textsize=output$textsize
  point=output$point
  family=output$family

  dados=output$ordempadronizado
  ana=output$anova
  nni=length(unique(dados$f1))
  respad=output$respAd
  ...
  if(design=="FAT2DIC.ad"){
    if(ana$`Pr(>F)`[3]>alpha.f & ana$`Pr(>F)`[2]<alpha.f){polynomial(f2,
                                                                     resp,
                                                                     grau = grau,
                                                                     ylab = ylab,
                                                                     xlab=xlab,
                                                                     posi=posi,
                                                                     theme=output$plot$graph1$theme,
                                                                     textsize=textsize,
                                                                     point=point,
                                                                     family=family,
                                                                     SSq = ana$`Sum Sq`[5],
                                                                     DFres = ana$Df[5])$plot}
    if(ana$`Pr(>F)`[3]<alpha.f){polynomial2_color(f2,resp,f1,grau = grau21,SSq = ana$`Sum Sq`[5],
                                                  theme=output$plot$interaction$theme,DFres = ana$Df[5])$plot}}
  if(design=="FAT2DBC.ad"){
    if(ana$`Pr(>F)`[4]>alpha.f & ana$`Pr(>F)`[2]<alpha.f){polynomial(f2,
                                                                     resp,
                                                                     grau = grau,
                                                                     ylab = ylab,
                                                                     xlab=xlab,
                                                                     ...
                                                                     SSq = ana$`Sum Sq`[6],
                                                                     DFres = ana$Df[6])$plot}
    if(ana$`Pr(>F)`[4]<alpha.f){polynomial2_color(f2,resp,f1,grau = grau21,SSq = ana$`Sum Sq`[6],,
                                                  theme=output$plot$interaction$theme,DFres = ana$Df[6])$plot}
  }
}
