...
FAT2DBC.ad=function(f1,
  ...
  { if(print.on==TRUE){
    ...
    {if(print.on==TRUE){
      ...
      if(quali[i]==TRUE){
        ...
        grafico=grafico+theme+ylab(ylab)+xlab(parse(text = lab.factor[i]))+ylim(ylim)
      ...
      if(quali[i]==FALSE){
        dose=as.numeric(as.vector(unlist(fatoresa[i])))
        grafico=polynomial(dose,
                           ...
                           textsize=textsize,
                           point=point,
                           family=family,SSq = anava$`Sum Sq`[6],
                           DFres = anava$Df[6],print.on = print.on,
                           plot.on = FALSE)
  ...
  if (anava$`Pr(>F)`[4]  <= alpha.f) {
    ...
    if(quali[1]==FALSE && color=="gray"| quali[2]==FALSE && color=="gray"){
      if(quali[2]==FALSE){
        Fator2=as.numeric(as.character(fator2a))#as.numeric(as.character(Fator2))
      ...
      if(quali[2]==TRUE){
        Fator1=as.numeric(as.character(fator1a))
        grafico=polynomial2(Fator1,
                            ...
                            print.on = print.on,
                            plot.on = FALSE)$plot+
          geom_hline(aes(lty=ad.label,yintercept=mean(responseAd,na.rm=TRUE)))+scale_linetype_manual(values=2)+labs(lty="")
        if(plot.on==TRUE){print(grafico)}
      }
    }
    if(quali[1]==FALSE && color=="rainbow"| quali[2]==FALSE && color=="rainbow"){
      if(quali[2]==FALSE){
        Fator2=as.numeric(as.character(fator2a))
  ...
  if(anava$`Pr(>F)`[4]>alpha.f){
    ...
    colints=graficos}else{colints=list(residplot=residplot,interaction=grafico)}
  output=list(plot=if(any(anava$`Pr(>F)`[c(1,2,4)] < alpha.f)){colints}else{NA},
              dec=dec,
              family=family,
              ylab=ylab,
              textsize=textsize,
              sup=sup,
              ordempadronizado=ordempadronizado,
              posi=posi,
              anova=anava,
              resp=resp,
              transf=transf,
