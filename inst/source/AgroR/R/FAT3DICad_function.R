...
#' @param names.fat Allows labeling the factors 1, 2 and 3 (this argument uses the \emph{parse} function).
...
FAT3DIC.ad = function(f1,
                      ...
                      point="mean_sd",
                      angle.label = 0) {
  lab.factor=c(gsub(" ","~",names.fat[1]),gsub(" ","~",names.fat[2]),gsub(" ","~",names.fat[3]))
  ...
  residplot=ggplot(data=data.frame(respad,Ids),aes(y=respad,x=1:length(respad)))+
    ...
    geom_hline(yintercept = c(0,-3,3),lty=c(1,2,2),color="red",linewidth=1)
  print(residplot)
  col1<-numeric(0)
  ...
  if(anavaF3[4,5]>alpha.f && anavaF3[5,5]>alpha.f && anavaF3[6,5]>alpha.f && anavaF3[7,5]>alpha.f) {
    ...
    for(i in 1:3){
      ...
      if(quali[i]==FALSE && anavaF3[i,5]<=alpha.f){
        ...
        grafico=polynomial(dose,resp,grau = grau[i],ylab = parse(text=ylab),xlab = parse(text=lab.factor[i]),
                           DFres= anavaF3[9,1],SSq = anavaF3[9,2],point = point)$plot
  ...
  if(anavaF3[7,5]>alpha.f && anavaF3[4,5]<=alpha.f){
    ...
    if(quali[1]==TRUE & quali[2]==TRUE){
      ...
      colint=ggplot(graph, aes(x=f2,
                               y=media,
                               fill=f1))+
         ylab(parse(text=ylab))+xlab(parse(text=lab.factor[2]))+
        theme+
        labs(fill=parse(text=lab.factor[1]))+
    ...
    if(quali[1]==FALSE | quali[2]==FALSE){
      ...
      if(quali[1]==FALSE){
        ...
        colint1=polynomial2(Fator1a,
                            ...
                            grau = grau12,
                            ylab=ylab,
                            xlab=parse(text=lab.factor[1]),legend.title=parse(text=lab.factor[2]),
                            theme=theme,
                            DFres= anavaF3[9,1],SSq = anavaF3[9,2])$plot}
      if(quali[2]==FALSE){
        if (mcomp == "tukey"){
          for (i in 1:nv2) {
            trati=fatores[, 1][Fator2 == lf2[i]]
      ...
      if(quali[2]==FALSE){
        ...
        colint1=polynomial2(Fator2a,
                            ...
                            grau = grau21,
                            ylab=ylab,
                            xlab=parse(text=lab.factor[2]),legend.title=parse(text=lab.factor[1]),
                            theme=theme,
                            DFres= anavaF3[9,1],SSq = anavaF3[9,2])$plot}
    ...
    if(anavaF3[5,5]>alpha.f && anavaF3[6,5]>alpha.f) {
      ...
      if(quali[i]==FALSE && anavaF3[i,5]<=alpha.f){
        ...
        cat(green(bold("\n------------------------------------------\n")))
        cat(fac.names[i])
        grafico1=polynomial(resp, fatores[,i],grau=grau[i],ylab = ylab,xlab = parse(text = lab.factor[3]),
    ...
    if(quali[1]==FALSE | quali[3]==FALSE){
      ...
      if(quali[1]==FALSE){
        ...
        colint2=polynomial2(Fator1a,
                            ...
                            grau = grau13,
                            ylab=ylab,
                            xlab=parse(text=lab.factor[1]),legend.title=parse(text=lab.factor[3]),
                            theme=theme,
                            DFres= anavaF3[9,1],SSq = anavaF3[9,2])$plot}
      if(quali[3]==FALSE){
        if (mcomp == "tukey"){
      ...
      if(quali[3]==FALSE){
        ...
        colint2=polynomial2(Fator3a,
                            ...
                            grau = grau31,
                            ylab=ylab,
                            xlab=parse(text=lab.factor[3]),legend.title=parse(text=lab.factor[1]),
                            theme=theme,
                            DFres= anavaF3[9,1],SSq = anavaF3[9,2])$plot}
      cat(green("\nTo edit graphical parameters, I suggest analyzing using the \"polynomial2\" command\n"))
    }
    ...
    if(anavaF3[4,5]>alpha.f && anavaF3[6,5]>alpha.f) {
      ...
      i<-2
      {
        ...
        if(quali[i]==FALSE && anavaF3[i,5]<=alpha.f){
          ...
          cat(green(bold("\n------------------------------------------\n")))
          cat(fac.names[i])
          grafico2=polynomial(resp, fatores[,i],grau=grau[i],ylab = ylab,xlab = parse(text = lab.factor[2]),
                              DFres= anavaF3[9,1],SSq = anavaF3[9,2],point = point)[[1]]
          print(grafico2)
          cat(green("\nTo edit graphical parameters, I suggest analyzing using the \"polynomial\" command"))
        }
  ...
  if(anavaF3[7,5]>alpha.f && anavaF3[6,5]<=alpha.f){
    ...
    if(quali[2]==TRUE & quali[3]==TRUE){
      ...
      colint=ggplot(graph, aes(x=f3,
                               ...
                               fill=f2))+
        geom_col(position = "dodge",color="black")+
         ylab(parse(text=ylab))+xlab(parse(text=lab.factor[3]))+
        theme+
        labs(fill=parse(text=lab.factor[2]))+
        geom_errorbar(aes(ymin=media-desvio,
                          ymax=media+desvio),
                      width=0.3,position = position_dodge(width=0.9))+
        geom_text(aes(y=media+desvio+sup,
    ...
    if(quali[2]==FALSE | quali[3]==FALSE){
      ...
      if(quali[2]==FALSE){
        ...
        colint3=polynomial2(Fator2a,
                            ...
                            grau = grau23,
                            ylab=ylab,
                            xlab=parse(text=lab.factor[2]),legend.title=parse(text=lab.factor[3]),
                            theme=theme,
                            DFres= anavaF3[9,1],SSq = anavaF3[9,2])$plot}
      if(quali[3]==FALSE){
        if (mcomp == "tukey"){
      ...
      if(quali[3]==FALSE){
        ...
        colint3=polynomial2(Fator3a,
                            ...
                            grau = grau32,
                            ylab=ylab,
                            xlab=parse(text=lab.factor[3]),legend.title=parse(text=lab.factor[2]),
                            theme=theme,
                            DFres= anavaF3[9,1],SSq = anavaF3[9,2])$plot}
    ...
    if(anavaF3[4,5]>alpha.f && anavaF3[5,5]>alpha.f) {
      ...
      i<-1
      {
        ...
        if(quali[i]==FALSE && anavaF3[i,5]<=alpha.f){
          ...
          cat(green(bold("\n------------------------------------------\n")))
          cat(fac.names[i])
          grafico3=polynomial(resp, fatores[,i],ylab = ylab,xlab = parse(text = lab.factor[1]),grau=grau[i],
                              DFres= anavaF3[9,1],SSq = anavaF3[9,2],point = point)[[1]]
          print(grafico3)
          cat(green("\nTo edit graphical parameters, I suggest analyzing using the \"polynomial\" command"))
        }
      ...
      for(j in 1:nv3) {
        ...
        if(quali[1]==FALSE){
          ...
          polynomial(fatores[,1][Fator2==lf2[i] & Fator3==lf3[j]],
                     ...
                     ylab = parse(text=ylab),xlab=parse(text = lab.factor[1]),point = point)[[1]]}
      ...
      for(j in 1:nv3) {
        ...
        if(quali[2]==FALSE){
          ...
          polynomial(fatores[,2][Fator1==lf1[k] & fatores[,3]==lf3[j]],
                     ...
                     DFres= anavaF3[9,1],SSq = anavaF3[9,2],ylab = parse(text=ylab),xlab=parse(text = lab.factor[2]),point = point)[[1]]}
      ...
      for(i in 1:nv2) {
        ...
        if(quali[3]==FALSE){
          ...
          polynomial(fatores[,3][fatores[,1]==lf1[k] & fatores[,2]==lf2[i]],
                     ...
                     DFres= anavaF3[9,1],SSq = anavaF3[9,2],ylab = parse(text=ylab),xlab=parse(text = lab.factor[3]),point = point)[[1]]}
      }
    }
  ...
  if(anavaF3[7,5]>alpha.f && anavaF3[5,5]<=alpha.f && anavaF3[6,5]<=alpha.f){
    ...
    graficos}
  if(anavaF3[7,5]>alpha.f && anavaF3[4,5]<=alpha.f && anavaF3[5,5]<=alpha.f && anavaF3[6,5]<=alpha.f){
    graficos=list(residplot,colint1,colint2,colint3)
    graficos}
