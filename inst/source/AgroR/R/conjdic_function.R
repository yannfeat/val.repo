...
conjdic=function(trat,
  ...
  print(homoge)
  cat("\n")
  message(if(homog1$p.value[1]>0.05){black("As the calculated p-value is greater than the 5% significance level, hypothesis H0 is not rejected. Therefore, the variances can be considered homogeneous")}
    ...
    for(i in 1:length(levels(local))){
        ...
        # if(quali==TRUE){
          ...
          if(mcomp=="sk"){
            ...
            nrep=table(data$tratamento)[1]
            medias=sort(tapply(data$resp,data$tratamento,mean),decreasing = TRUE)
            letra=scottknott(means = medias,
                             df1 = anova3$Df[2],
                             nrep = nrep,
                             QME = anova3$`Mean Sq`[2],
                             alpha = alpha.t)
