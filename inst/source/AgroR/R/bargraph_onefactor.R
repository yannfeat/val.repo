...
bargraph_onefactor=function(analysis,
  ...
  for(i in 1:length(analysis)){
    if(is.null(labels)==TRUE){analysis[[i]]$dadosm$facet=rep(i, e=nrow(analysis[[i]]$dadosm))}else{
      analysis[[i]]$dadosm$facet=rep(labels[i],e=nrow(analysis[[i]]$dadosm))}
    results[[i]]=analysis[[i]]$dadosm}
  ...
  graph=ggplot(tabela,aes(y=media,x=trats))+
    geom_col(color="black",fill="lightblue",width = analysis$width.column)+
    ...
    analysis$plot$theme+theme(strip.text = element_text(size=facet.size))
  if(is.null(ylab)==TRUE){graph=graph+ylab(analysis$plot$labels$y)}else{graph=graph+ylab(parse(text=ylab))}
