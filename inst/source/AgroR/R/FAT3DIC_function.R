    ...
    for(i in 1:3){
      if(quali[i]==TRUE && anavaF3[i,5]<=alpha.f) {
        ...
        if(geom=="bar"){grafico=ggplot(dadosm,
                                       aes(x=trats,
                                           y=media))
        if(fill[1]=="trat"){grafico=grafico+
          geom_col(aes(fill=trats),
                   color=1)}else{grafico=grafico+
                     geom_col(fill=fill[1],color=1)}
      ...
      if(quali[i]==FALSE && anavaF3[i,5]<=alpha.f){
        ...
        grafico=polynomial(dose,resp,grau = grau[i],
                           DFres= anavaF3[8,1],SSq = anavaF3[8,2],ylab = ylab,xlab = parse(text = lab.factor[i]),point = point)[[1]]
      ...
      if(quali[1] & quali[2]==TRUE){
          ...
          colint=ggplot(graph,
            ...
            theme(text=element_text(size=textsize,family=family),
                  axis.text = element_text(size=textsize,color="black",family=family),
                  axis.title = element_text(size=textsize,color="black",family=family))
          if(length(fill)>=2){colint=colint+scale_fill_manual(values=fill)}
          colint1=colint
          print(colint)
      ...
      if(quali[1]==FALSE | quali[2]==FALSE){
        ...
        if(quali[1]==FALSE){
            ...
            colint1=polynomial2(Fator1,
                                ...
                                theme=theme,
                                DFres= anavaF3[8,1],SSq = anavaF3[8,2])$plot}
        if(quali[2]==FALSE){
          if (mcomp == "tukey"){
            for (i in 1:nv2) {
              trati=fatores[, 1][Fator2 == lf2[i]]
        ...
        i<-3
        {
          ...
          if(quali[i]==FALSE && anavaF3[i,5]<=alpha.f){
            ...
            grafico1=polynomial(fatores[,i], resp,grau=grau[i],
                                DFres= anavaF3[8,1],SSq = anavaF3[8,2],ylab = ylab,xlab = parse(text = lab.factor[3]),point = point)[[1]]
            cat(green("\nTo edit graphical parameters, I suggest analyzing using the \"polynomial\" command"))}
        }
          ...
          i<-2
          {
            ...
            if(quali[i]==FALSE && anavaF3[i,5]<=alpha.f){
              ...
              grafico2=polynomial(fatores[,i], resp,grau=grau[i],
                                  DFres= anavaF3[8,1],SSq = anavaF3[8,2],ylab = ylab,xlab = parse(text = lab.factor[2]),point = point)[[1]]
              cat(green("\nTo edit graphical parameters, I suggest analyzing using the \"polynomial\" command"))
            }
          ...
          i<-1
          {
            ...
            if(quali[i]==FALSE && anavaF3[i,5]<=alpha.f){
              ...
              grafico3=polynomial(fatores[,i], resp,grau=grau[i],
                         DFres= anavaF3[8,1],SSq = anavaF3[8,2],ylab = ylab,xlab=parse(text = lab.factor[1]),point = point)[[1]]
              print(grafico3)
              cat(green("\nTo edit graphical parameters, I suggest analyzing using the \"polynomial\" command"))
            }
        ...
        for(j in 1:nv3) {
          ...
          if(quali[2]==FALSE){
            ...
            polynomial(fatores[,2][Fator1==lf1[k] & fatores[,3]==lf3[j]],
                       ...
                       ylab = ylab,xlab = parse(text = lab.factor[2]),point = point)[[1]]}
          }
      }
