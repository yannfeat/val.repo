...
#' @seealso \link{barplot_positive}, \link{plot_TH}, \link{plot_TH1}, \link{corgraph}, \link{spider_graph}, \link{line_plot}, \link{plot_cor}, \link{plot_interaction}, \link{plot_jitter}, \link{seg_graph}, \link{TBARPLOT.reverse}
#' @examples
#' data("laranja")
...
bar_graph2=function(model,
  ...
  graph=ggplot(data,aes(x=trats,
                        y=media))+
    model$plot$theme+
    geom_col(fill=fill,width=model$width.column,color="black")+
    ...
    geom_text(aes(y=media+desvio+sup,
                  x=trats,
                  label = letra),vjust=0,size=model$labelsize,color=label.color,
              family=model$family)+
    geom_text(aes(y=y.text,
                  x=trats,
                  label = trats),hjust=0,angle=90,size=model$labelsize,color=text.color,
              family=model$family)+
    labs(x=model$plot$labels$x,
         y=model$plot$labels$y)+
