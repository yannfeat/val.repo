...
DBC=function(trat,
  ...
  if(quali==FALSE){
    trat=trat1
    if(grau==1){graph=polynomial(trat,response, grau = 1,xlab=parse(text=xlab),ylab=parse(text=ylab),textsize=textsize, family=family,posi=posi,point=point,SSq = a$`Sum Sq`[3],DFres = a$Df[3],pointsize = pointsize,print.on = print.on,plot.on = FALSE)}
    if(grau==2){graph=polynomial(trat,response, grau = 2,xlab=parse(text=xlab),ylab=parse(text=ylab),textsize=textsize, family=family,posi=posi,point=point,SSq = a$`Sum Sq`[3],DFres = a$Df[3],pointsize = pointsize,print.on = print.on,plot.on = FALSE)}
    if(grau==3){graph=polynomial(trat,response, grau = 3,xlab=parse(text=xlab),ylab=parse(text=ylab),textsize=textsize, family=family,posi=posi,point=point,SSq = a$`Sum Sq`[3],DFres = a$Df[3],pointsize = pointsize,print.on = print.on,plot.on = FALSE)}
    grafico=graph[[1]]
    if(is.na(ylim[1])==FALSE){
      grafico=grafico+scale_y_continuous(breaks = ylim,
                                         limits = c(min(ylim),max(ylim)))}
  ...
  output=list(
      ...
      anova=if(test=="parametric"){a}else{NA},
      fried=if(test=="noparametric"){fried$statistics}else{NA},
