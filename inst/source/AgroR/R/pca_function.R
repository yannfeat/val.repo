...
#' @param ylab Names y-axis (this argument uses the \emph{parse} function)
#' @param xlab Names x-axis (this argument uses the \emph{parse} function)
...
PCA_function=function(data,
  ...
  if(is.na(xlab)==TRUE){xlab=paste("PC1 (",round(autov$PC1[2]*100,2),"%)",sep = "")}else{ylab=parse(text=gsub(" ","~",ylab))}
  if(is.na(ylab)==TRUE){ylab=paste("PC2 (",round(autov$PC2[2]*100,2),"%)",sep = "")}else{xlab=parse(text=gsub(" ","~",xlab))}
  # Avt[,1]=Avt[,1]*-1
