...
DIC <- function(trat,
  ...
  if(test=="parametric"){
    ...
    if(plot.on==TRUE){print(residplot)}
    if(print.on==TRUE){
    cat(green(bold("\n-----------------------------------------------------------------\n")))
    ...
    if(print.on==TRUE){print(anava1,na.print = "")
    cat("\n\n")
    ...
    if(quali==TRUE){
      ...
      if(print.on==TRUE){print(if(a$`Pr(>F)`[1]<alpha.f){letra1}else{"H0 is not rejected"})
      cat("\n")
    ...
    if(quali==FALSE){
      ...
      if(grau==1){graph=polynomial(trat,response, grau = 1,textsize=textsize,xlab=parse(text=xlab),ylab=parse(text=ylab), family=family,posi=posi,point=point,pointsize = pointsize,print.on = print.on,plot.on = FALSE)}
      if(grau==2){graph=polynomial(trat,response, grau = 2,textsize=textsize,xlab=parse(text=xlab),ylab=parse(text=ylab), family=family,posi=posi,point=point,pointsize = pointsize,print.on = print.on,plot.on = FALSE)}
      if(grau==3){graph=polynomial(trat,response, grau = 3,textsize=textsize,xlab=parse(text=xlab),ylab=parse(text=ylab), family=family,posi=posi,point=point,pointsize = pointsize,print.on = print.on,plot.on = FALSE)}
      grafico=graph[[1]]
      if(is.na(ylim[1])==FALSE){
        grafico=grafico+scale_y_continuous(breaks = ylim,
                                           limits = c(min(ylim),max(ylim)))}
      if(plot.on==TRUE){print(grafico)}
    }}
  if(test=="noparametric"){
    ...
    kruskal=function (y,
      ...
      if(console){cat(paste(name.t, ",", sep = ""), " means of the ranks\n\n")
        if(print.on==TRUE){print(data.frame(row.names = means[, 1], means[, -1]))}
        cat("\nPost Hoc Analysis\n")}
      if (p.adj != "none") {
  ...
  if(quali==TRUE){if(plot.on==TRUE){print(grafico)}}
  output=list(plot=grafico,
       resp=resp,
       response=response,
       trat=trat,
       test=test,
       ...
       ylab=ylab,
       family=family,
       dadosm=if(quali==T & (geom == "point" | geom=="bar")){dadosm}else{if(geom=="box"){dadosm2}else{NA}},
       anova=if(test=="noparametric"){NA}else{a},
       krusk=if(test=="noparametric"){krusk}else{NA},
       norm=if(test=="parametric"){norm1}else{NA},
       homog=if(test=="parametric"){homog1}else{NA},
       indep=if(test=="parametric"){indep}else{NA})
  # graficos=grafico#list(grafico)#[[1]]
}
