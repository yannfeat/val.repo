...
summarise_conj=function(analysis,design="DBC",info="p"){
  if(design=="DIC"){
    ...
    for(i in 1:length(analysis)){
      ...
      ##########################################
      # teste medias geral
      if(qmres[i]<analysis[[i]]$homog.value & ano$`Pr(>F)`[3]>analysis[[i]]$alpha.f){
        compa=analysis[[i]]$dadosm[,"letra"]}else{
          compa=rep("-",e=length(unique(analysis[[i]]$trat)))
        }
      comp[,i]=compa
  ...
  if(design=="DBC"){
    ...
    comp=data.frame(matrix(rep(rep("-",
                   length(unique(analysis[[1]]$trat))),length(analysis)),
                   ncol = length(analysis)))
    for(i in 1:length(analysis)){
    nomes[i]=analysis[[i]]$ylab
    qmres[i]=analysis[[i]]$qmresmedio
    ano=analysis[[i]]$datas

    ##########################################
    # teste medias geral
    if(qmres[i]<analysis[[i]]$homog.value & ano$`Pr(>F)`[4]>analysis[[i]]$alpha.f){
      compa=analysis[[i]]$dadosm[,"letra"]}else{
        compa=rep("-",e=length(unique(analysis[[i]]$trat)))
      }
    comp[,i]=compa
    ...
    data=data.frame(data)
    colnames(data)=nomes
    rownames(comp)=rownames(analysis[[1]]$dadosm)
    colnames(comp)=nomes
    # list("Anova"=data,
    #      "Post-Hoc"=comp)
  }
