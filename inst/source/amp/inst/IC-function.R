...
ic.func.est <- function(obs_dat, what = "both", control = NULL) {
  ...
  both <- causalNullTest(Y, A, W, p = NA,
                         control = list(cross.fit = crsfit, V = V,
                                        ...
                                        g.n.bins = 2:5),
                         return_ic_est = TRUE)
  if (!is.null(control$grid.mult)) {
    num.gpnts <- round(control$grid.mult * ncol(both$ic))
    keep_ids <- sample(1:ncol(both$ic), num.gpnts, replace = FALSE)
    both$est <- both$est[keep_ids]
    both$ic <- both$ic[, keep_ids]
  }
  fin <- list()
