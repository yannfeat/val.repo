...
function (Ps, Ts, ws, Psurface, deltaP, PWIDTH, doLog = 0) 
{
    ...
    ws2 <- na.omit(ws)
    nlevs <- length(Ts2)
    outvals <- .C("LIindex_Rworld", as.double(Ps2), as.double(Ts2), 
        as.double(ws2), as.integer(nlevs), as.double(Psurface), 
        as.integer(doLog), as.double(deltaP), as.double(PWIDTH), 
        result = double(1))
