#' @title Cross-Validation for Temkin Isotherm Non-Linear Model with Clustering-based Fold Assignment
#' @name cv_temkinNLM
#' @description
#' Performs K-fold or leave-one-out cross-validation (LOOCV) on the non-linear Temkin isotherm model:
#' Qe = (RT / bT) * log(AT * Ce)
...
#' @param Temp Numeric scalar. Temperature in Kelvin. Must be positive.
...
cv_temkinNLM <- function(Ce, Qe, Temp, K = 10, seed = 123, loocv = FALSE, plot = FALSE, use_clustering = FALSE) {
  ...
  if (!is.numeric(Temp) || Temp <= 0) {
    stop("Temperature (Temp) must be a positive numeric scalar.")
  }
  ...
  for (i in 1:K) {
    ...
    start_vals <- data.frame(AT = seq(0.01, 5, length = 10), bT = seq(1, 100, length = 10))
