#' @title Cross-Validation for Freundlich Isotherm Linear Model with Clustering-based Fold Assignment
#' @name cv_freundlichLM
#' @description
#' Performs K-fold or leave-one-out cross-validation (LOOCV) on the linearized Freundlich isotherm model:
...
cv_freundlichLM <- function(Ce, Qe, K = 10, seed = 123, loocv = FALSE, plot = FALSE, use_clustering = FALSE) {
  if (length(Ce) != length(Qe)) {
    stop("Ce and Qe must be of the same length.")
  }
  if (any(Ce <= 0) || any(Qe <= 0)) {
    stop("All Ce and Qe values must be positive for log transformation.")
  }
  ...
  set.seed(seed)
  data <- data.frame(log_Ce = log(Ce), log_Qe = log(Qe))

  if (use_clustering && !loocv) {
    # Cluster samples into K groups on log-transformed data
    clusters <- stats::kmeans(data, centers = K, nstart = 25)$cluster

    # Now assign folds by distributing clusters evenly across folds
    # Shuffle cluster IDs and assign folds within each cluster to maintain balance
    folds <- integer(n)
    for (cl in unique(clusters)) {
      idx <- which(clusters == cl)
      n_cl <- length(idx)
      # Assign fold numbers evenly within cluster
  ...
  for (i in 1:K) {
    ...
    model <- stats::lm(log_Qe ~ log_Ce, data = train_data)
    preds <- stats::predict(model, newdata = test_data)

    fold_mse[i] <- mean((test_data$log_Qe - preds)^2)
  }
