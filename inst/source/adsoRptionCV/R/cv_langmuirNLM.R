#' @title Cross-Validation Analysis for Langmuir Isotherm Non-linear Model with Clustering-based Fold Assignment
#' @name cv_langmuirNLM
#' @description
#' Performs K-fold or leave-one-out cross-validation (LOOCV) on the Langmuir isotherm non-linear model:
#' Qe = (Qmax * Ce) / (b + Ce)
#' where Qmax is the maximum adsorption capacity and b is the Langmuir constant.
#' Supports optional clustering-based fold assignment and visualization of fold MSEs.
#'
...
utils::globalVariables(c("Ce", "Qe", "log_Ce", "log_Qe"))

cv_langmuirNLM <- function(Ce, Qe, K = 10, seed = 123, loocv = FALSE, plot = FALSE, use_clustering = FALSE) {
  ...
  if (any(Ce <= 0) || any(Qe <= 0)) {
    stop("All Ce and Qe values must be positive.")
  }
  ...
  if (loocv) {
    ...
    stop(sprintf("Number of folds K (%d) cannot exceed number of observations (%d).", K, n))
  }
  ...
  for (i in 1:K) {
    ...
    model_formula <- Qe ~ (Qmax * Ce) / (b + Ce)
    start_vals <- data.frame(Qmax = seq(max(Qe) * 0.8, max(Qe) * 1.2, length = 5),
                             b = seq(0.1, 10, length = 5))
