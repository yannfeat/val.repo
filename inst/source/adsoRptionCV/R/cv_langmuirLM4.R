#' @title Cross-Validation for Langmuir Isotherm Fourth Linear Model with Clustering-based Fold Assignment
#' @name cv_langmuirLM4
#' @description
#' Performs K-fold or leave-one-out cross-validation (LOOCV) on the linearized Langmuir isotherm model (Form 4):
#' Qe / Ce = b * Qmax - b * Qe
#' This corresponds to a linear regression of Qe / Ce versus Qe.
...
utils::globalVariables(c("Qe", "Qe_over_Ce"))

cv_langmuirLM4 <- function(Ce, Qe, K = 10, seed = 123, loocv = FALSE, plot = FALSE, use_clustering = FALSE) {
  if (length(Ce) != length(Qe)) {
    stop("Ce and Qe must be of the same length.")
  }
  if (any(Ce <= 0) || any(Qe <= 0)) {
    stop("All Ce and Qe values must be positive for transformation.")
  }
  ...
  set.seed(seed)
  data <- data.frame(Qe = Qe, Qe_over_Ce = Qe / Ce)
  ...
  for (i in 1:K) {
    ...
    model <- tryCatch({
      stats::lm(Qe_over_Ce ~ Qe, data = train_data)
    }, error = function(e) {
    ...
    fold_mse[i] <- mean((test_data$Qe_over_Ce - preds)^2)
  }
