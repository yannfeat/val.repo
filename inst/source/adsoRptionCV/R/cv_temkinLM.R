#' @title Cross-Validation for Temkin Isotherm Linear Model with Clustering-based Fold Assignment
#' @name cv_temkinLM
#' @description
#' Performs K-fold or leave-one-out cross-validation (LOOCV) on the Temkin isotherm linear model:
#' Qe = B * log(A) + B * log(Ce)
#' This corresponds to a linear regression of Qe versus log(Ce).
...
utils::globalVariables(c("Qe", "log_Ce"))

cv_temkinLM <- function(Ce, Qe, K = 10, seed = 123, loocv = FALSE, plot = FALSE, use_clustering = FALSE) {
  if (length(Ce) != length(Qe)) {
    stop("Ce and Qe must be of the same length.")
  }
  if (any(Ce <= 0)) {
    stop("All Ce values must be positive for log transformation.")
  }
  ...
  set.seed(seed)
  data <- data.frame(log_Ce = log(Ce), Qe = Qe)
  ...
  for (i in 1:K) {
    ...
    model <- tryCatch({
      stats::lm(Qe ~ log_Ce, data = train_data)
    }, error = function(e) {
