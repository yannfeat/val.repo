#' @title Cross-Validation for Langmuir Isotherm Third Linear Model with Clustering-based Fold Assignment
#' @name cv_langmuirLM3
#' @description
#' Performs K-fold or leave-one-out cross-validation (LOOCV) on the linearized Langmuir isotherm model (Form 3):
#' Qe = Qmax - (1 / b) * (Qe / Ce)
#' This corresponds to a linear regression of Qe versus Qe / Ce.
...
cv_langmuirLM3 <- function(Ce, Qe, K = 10, seed = 123, loocv = FALSE, plot = FALSE, use_clustering = FALSE) {
  ...
  for (i in 1:K) {
    ...
    model <- tryCatch({
      stats::lm(Qe ~ Qe_over_Ce, data = train_data)
    }, error = function(e) {
