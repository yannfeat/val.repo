#' @title Cross-Validation for Langmuir Isotherm First Linear Model with Clustering-based Fold Assignment
#' @name cv_langmuirLM1
#' @description
#' Performs K-fold or leave-one-out cross-validation (LOOCV) on the linearized Langmuir isotherm model (Form 1):
#' Ce / Qe = 1 / (Qmax * b) + Ce / Qmax
#' This corresponds to a linear regression of Ce / Qe versus Ce.
...
utils::globalVariables(c("Ce", "Ce_over_Qe"))

cv_langmuirLM1 <- function(Ce, Qe, K = 10, seed = 123, loocv = FALSE, plot = FALSE, use_clustering = FALSE) {
  ...
  set.seed(seed)
  data <- data.frame(Ce = Ce, Ce_over_Qe = Ce / Qe)
  ...
  for (i in 1:K) {
    ...
    model <- tryCatch({
      stats::lm(Ce_over_Qe ~ Ce, data = train_data)
    }, error = function(e) {
    ...
    fold_mse[i] <- mean((test_data$Ce_over_Qe - preds)^2)
  }
